...
Rather than demand a docstring on every symbol — the coverage-threshold
trap that fires on trivial one-liners — this scores a function's
documentation *value* from its cognitive complexity and signature and
warns only when a non-trivial public function is under-documented. It
reuses RS012's cognitive-complexity scorer read at a lower threshold, so
the signal rides an existing metric rather than a new one. The warning
reads as "documentation would help here," distinct from a binary
"missing docstring" error.

The signal has three triggers. The presence trigger fires when a complex
or many-argumented public function carries no docstring at all. The
`Args:` trigger fires when a documented public function with many
parameters states them in prose rather than a structured `Args:`
section, nudging toward the scannable form. The `Returns:` trigger fires
when a documented function returns a multi-element `tuple` — an
anonymous composite whose parts a single summary line cannot name. A
scalar, a named type (which self-documents via its per-field docstrings,
RS004), or a homogeneous collection is left to the imperative summary
line ("Return the ..."), so no `Returns:` block is asked of it.
"""
...
# A function clears the value gate for the presence check when it scores
# at or above the complexity floor (well below RS012's limit of 15, which
# marks over-complexity rather than mere worth-documenting) or carries at
# least the parameter floor. The `Args:` trigger has its own parameter
# floor: the count above which a structured section beats prose.
...
def _returns_multi_element_tuple(node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
    ...
    A multi-element `tuple[A, B, ...]` is an anonymous composite a single
    summary line cannot enumerate, so it warrants a `Returns:` section.
    The variadic `tuple[X, ...]` form is a homogeneous sequence, not a
    composite, and is excluded.
    """
...
def check_doc_value_signal(path: Path, source: str) -> Iterator[Violation]:
    ...
    multi-element `tuple` but no `Returns:` section. Trivial functions and
    non-public, test, and `@overload` definitions never fire — the
    false-positive engine this rule exists to avoid.
    """
