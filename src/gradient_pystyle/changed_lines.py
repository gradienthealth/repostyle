...
request on the pre-existing backlog. A whole-unit finding (a future
complexity rule reported at the `def` line) re-arms only when that
reported line itself changes, not when the body around it does — the
intersection is purely on the violation's own line, which keeps the
pre-existing backlog from re-arming on any incidental edit.
"""
...
def changed_lines(path: Path, base: str) -> set[int] | None:
    ...
    diff = _run_git(["diff", "--unified=0", base, "--", path.name], directory)
    ...
    for line in diff.stdout.splitlines():
        ...
        if header is not None:
            ...
            new_lineno += 1
    if not lines:
        tracked = _run_git(["ls-files", "--error-unmatch", "--", path.name], directory)
