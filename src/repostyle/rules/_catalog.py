...
RULE_DOCS: dict[str, RuleDoc] = {
    ...
    RS_DISFAVORED_GCP_TERM: RuleDoc(
        ...
        rationale=(
            ...
            "as a whole word, so a substring (`GCS` in `GCSError`) or a term "
            "glued to a hyphen is left alone; a term in code font (the "
    ...
    RS_GCP_BARE_IDENTIFIER: RuleDoc(
        ...
        rationale=(
            ...
            "the value; it also reads as the `project_id` the code should feed a "
            "Pulumi `project=` argument, over the bare `project` Pulumi's own "
            "signature spells. The check fires only on a string annotation, so a "
