...
RULE_DOCS: dict[str, RuleDoc] = {
    ...
    RS_DEEPLY_NESTED_TYPE: RuleDoc(
        ...
        rationale=(
            ...
            "a name. A single-level `Iterator[tuple[...]]` or `dict[str, "
            "list[...]]` is idiomatic and left alone; every subscript layer "
            "beyond that counts the same — a `tuple`, a `Callable`, or a union "
            "is no easier to read nested — so the remedy is to give the inner "
