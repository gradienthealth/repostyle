...
1. The Pulumi logical name, a resource's first positional argument. It is the program-local URN handle, neither a cloud id nor a path.
...
- `*_resource_name` and `*_path` are a full `{collection}/{id}` path, such as `projects/{project}` or `buckets/{bucket}/objects/{object}`. This is the form an IAM condition or a fully-qualified reference needs.
- `logical_name` is the Pulumi URN name, a resource's first positional argument. Never call it `*_name`, which reads as a cloud identifier, nor `resource_name`, which AIP-122 already spends on the full path above.
