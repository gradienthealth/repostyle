...
| RS039 | Unbackticked sibling symbol (warning): where a docstring already backticks one code symbol, a bare sibling token in the same docstring stays bare. To keep the false-positive rate near zero it fires only past two guards — the docstring must already backtick a code-shaped token, and the bare token must recur verbatim inside a string literal in the same file (a table or column name in an embedded SQL statement, say) as self-contained proof it names a real identifier. A name the module binds is left to RS036, so the two never flag one token. |
