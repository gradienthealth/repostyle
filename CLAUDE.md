...
- `src/repostyle/_shared.py`, `src/repostyle/_comments.py` — package-level shared infrastructure both the rule modules and the top-level `runner` / `suppressions` consume, so they sit above the `rules` package rather than one level down inside it (their old home reached into by RS048). `_shared` holds the cross-module helpers (`find_pyproject`, the `[tool.repostyle]` config-table and `.gitignore` readers, `_parse_python`); `_comments` the per-language `#`-comment extractor (`extract_comments`, `COMMENT_SUFFIXES`) RS009 / RS022 / RS030 and the suppression parser share — Python via the tokenizer, TOML and YAML via a string- and block-aware line scan.
