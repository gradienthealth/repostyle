...
A stdlib-only AST/token/line linter for repo-style conventions that ruff cannot express, shared across gradienthealth repos and run as a pre-commit remote hook. It is the extraction of fhir-ingestor's `scripts/check_repo_style.py` into a versioned package, so the `RSnnn` rules here are the canonical source the other repos consume. Most rules are Python-specific (AST-based); the comment-convention rules RS009 (paragraph wrapping), RS022 (tag format), RS030 (terminal punctuation), RS045 (temporal / diff-narrative markers), RS049 (acronym casing in prose), and RS050 (disfavored Google Cloud product name) also run over TOML, YAML, and shell comments through a shared `#`-comment extractor, so a `#` comment is held to the same conventions whatever the language.
...
- `src/repostyle/rules/` — the rules package. Each themed module holds the `check_*` functions for its rules; a rule's full definition and rationale live in its check-function docstring, so read that before changing behavior — the list below maps where each rule lives, it is not a spec. Themed rule modules:
  ...
  - `comments` — RS022 (comment-tag format), RS038 (a wrapped tag comment indents its continuation past the tag; warn), the comment half of RS030, the comment half of RS045 (temporal / diff-narrative marker; warn), the comment half of RS049 (acronym casing in prose; warn, fix), and the comment half of RS050 (disfavored Google Cloud product name; warn, fix), all run over Python, TOML, YAML, and shell comments
